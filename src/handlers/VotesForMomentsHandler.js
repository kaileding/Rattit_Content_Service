 ...
 * @Last Modified time: 2017-08-10 01:09:44
*/
	...
	deleteVoteForMomentByContent(voteObj) {
		return new Promise((resolve, reject) => {
			...
			model.destroy({
				...
				if (response === 1) {
                	cLogger.say(`Deleted entry with vote_type '${voteObj.vote_type}', moment_id '${voteObj.moment_id}' and createdBy '${voteObj.createdBy}' from ${model.name}`);
                    resolve("OK");
                } else if (response === 0) {
                	cLogger.debug(`Unable to delete nonexistent entry with vote_type '${voteObj.vote_type}', moment_id '${voteObj.moment_id}' and createdBy '${voteObj.createdBy}' in ${model.name}`);
